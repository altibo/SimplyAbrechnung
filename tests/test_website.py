...
def test_download_website_structure_and_assets():
    ...
    assert {"version-label", "windows-download", "mac-download", "release-meta", "release-link"} <= set(parser.ids)
    assert parser.scripts == ["app.js"]
    assert parser.stylesheets == ["styles.css"]
    assert all((DOCS / asset).is_file() for asset in parser.scripts + parser.stylesheets)
